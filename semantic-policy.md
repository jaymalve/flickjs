...
This file shows one concrete example of how semantic policy refinement would be implemented in Zarc.
...
Suppose Zarc currently flags this:
...
After the unit tests pass, run Zarc on a few real projects and inspect:
...
Semantic policy refinement in Zarc is this loop:
...
This is how Zarc becomes trustworthy.
...
Suppose Zarc currently flags simple cases like:
...
Define what Zarc means by "prefer const".
...
Those are different product decisions. The semantic layer enables both, but Zarc has to choose one.

## Step 4: Validate On Real Code

Run Zarc on real repos and look specifically for:
